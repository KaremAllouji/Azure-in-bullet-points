...
- You can integrate Azure CDN
  ...
  - allows you to improve performance by removing the burden of serving static, unchanging files from the main server to a network of servers around the globe; a CDN can reduce traffic to a server by 50% or more, which means you can serve more users or serve the same users faster
    - Distributed network of cache servers
    - it caches the files from the storage acc. for quick load/answers
  ...
  - supports multiple CDN endpoints (with storages...)
    - Azure Storage blob becomes ***origin server***. (point 1)
    - Azure CDN servers become ***edge servers***.    (point 2)
  ...
  - ***Set up***: Two alternatives
    - Create CDN and configure against blob service (endpoint).
...
- SMB File Shares (shared access to Volumes)
- Attach to Virtual Machines as file shares
- **Problem**: Many ISP block port 445 which is being used for SMB -> **solution**:Integration with Azure File Sync
...
- **Convert to ZRS with a live migration from Azure Support**
  - only for Standard-notBlobStorage- LRS supported.
  - Premium -> manual migration.
...
- You can use physical drives
  ...
  - **WAImportExportTool**
    - Prepares the harddisk env. for uploading to Azure through Data Box Service
  ...
  - **Azure Databox**
    - Microsoft ships Data Box storage device (Offline)
      - Each storage Box has a maximum usable storage capacity of 100 TB.
      - Each storage Disk(SSD) has a maximum usable storage capacity of 8 TB.
      - Each storage Heavy has a maximum usable storage capacity of 1 PB.
